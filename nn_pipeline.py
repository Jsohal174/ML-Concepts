...
helper_utils3.print_data_folder_structure(path_dataset, max_depth=1)
...
helper_utils3.plot_img(img, label=label, info=img_size_info)
...
def visual_exploration(dataset, num_rows=2, num_cols=4):
    ...
    fig, axes = helper_utils3.get_grid(num_rows, num_cols, figsize=(num_cols * 3, num_rows * 4))
    ...
    for ax, idx in zip(axes.flatten(), indices):
        ...
        helper_utils3.plot_img(image, label=label, info=info, ax=ax)
...
# quick check
helper_utils3.quick_debug(img_transformed)

# Plot the transformed image
helper_utils3.plot_img(img_transformed, label=label)
...
helper_utils3.plot_img(img_tensor, label=label, info=img_shape_info)
...
for epoch in range(n_epochs):
    ...
    train_bar = helper_utils3.get_dataloader_bar(train_dataloader, color='blue')
    ...
    for batch, (images, labels) in enumerate(train_dataloader):
        ...
        helper_utils3.update_dataloader_bar(train_bar, batch, batch_size, train_samples)
    ...
    val_bar = helper_utils3.get_dataloader_bar(val_dataloader, color='orange')
    ...
    for batch, (images, labels) in enumerate(val_dataloader):
        ...
        helper_utils3.update_dataloader_bar(val_bar, batch, batch_size, val_samples)
...
test_bar = helper_utils3.get_dataloader_bar(test_dataloader, color='green')
...
for batch, (images, labels) in enumerate(test_dataloader):
    ...
    helper_utils3.update_dataloader_bar(test_bar, batch, batch_size, test_samples)
...
def visualize_augmentations(dataset_aug, idx=0, num_versions=8):
    ...
    fig, axes = helper_utils3.get_grid(n_rows, n_cols, figsize=(16, 8))
    ...
    for ax in axes.flatten():
        ...
        helper_utils3.plot_img(img=img, ax=ax)
